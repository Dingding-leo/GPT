...
def _cached_backtest_selected_position_paths(
    payload: Mapping[str, object],
    persisted: pd.DataFrame,
    source_close: pd.Series,
) -> tuple[int, int]:
    """Reproduce the previous cached verifier implementation for benchmarking."""
...
def main() -> None:
    ...
    def baseline() -> list[dict[str, object]]:
        return _run_verifier(root, _cached_backtest_selected_position_paths)

    def optimized() -> list[dict[str, object]]:
        return _run_verifier(root, verify_gate._validate_selected_position_paths)

    if baseline() != optimized():
        raise AssertionError("target-only verifier output differs from cached-backtest baseline")
    ...
    output = {
        ...
        "baseline": "cached_full_backtest_target_position_path",
        "optimized": "cached_target_only_delayed_position_path",
