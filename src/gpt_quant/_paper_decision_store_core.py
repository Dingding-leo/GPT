...
_MAX_DECISION_RECORD_BYTES = _MAX_SERIALIZED_BYTES
_MAX_DECISION_STORE_ENTRIES = 131_072
_MAX_DECISION_STORE_RECORDS = 65_536
...
def _store_lock(directory_descriptor: int) -> Iterator[None]:
    """Serialize store-wide replay and publication on the pinned directory inode."""

    if _fcntl is None:
        raise RuntimeError("paper decision locking requires POSIX advisory locks")
    try:
        _fcntl.flock(directory_descriptor, _fcntl.LOCK_EX | _fcntl.LOCK_NB)
    except BlockingIOError as exc:
        raise RuntimeError(f"{_ERROR} lock is held") from exc
    try:
        yield
    finally:
        _fcntl.flock(directory_descriptor, _fcntl.LOCK_UN)
...
def _record_paper_order_decision_unlocked(
    ...
    *,
    ...
    if len(payload) > _MAX_DECISION_RECORD_BYTES:
...
def _bounded_decision_names(directory_descriptor: int) -> tuple[str, ...]:
    decision_names: list[str] = []
    entry_count = 0
    with os.scandir(directory_descriptor) as entries:
        for entry in entries:
            entry_count += 1
            if entry_count > _MAX_DECISION_STORE_ENTRIES:
                raise ValueError(f"{_ERROR} store exceeds the maximum entry count")
            if not entry.name.endswith(".json"):
                continue
            if len(decision_names) >= _MAX_DECISION_STORE_RECORDS:
                raise ValueError(f"{_ERROR} store exceeds the maximum decision count")
            decision_names.append(entry.name)
    return tuple(sorted(decision_names))
...
def _replay_paper_order_decision_store_unlocked(
    ...
    for name in _bounded_decision_names(directory_descriptor):
