...
class PaperOrderDecision:
    ...
    def __post_init__(self) -> None:
        ...
        exchange_fee_bps = _decimal(self.exchange_fee_bps, "exchange_fee_bps")
        if exchange_fee_bps != _CANONICAL_EXCHANGE_FEE_BPS:
            raise ValueError("exchange_fee_bps must be exactly 5")
        object.__setattr__(self, "exchange_fee_bps", exchange_fee_bps)
        for name in (
            "spread_bps",
            "slippage_bps",
            "market_impact_bps",
