...
def verify_walk_forward_report(
    ...
    *,
    ...
    """Recompute persisted 5 bps walk-forward evidence and fail closed on drift."""
    ...
    if not math.isclose(fee_bps, _BASELINE_FEE_BPS, rel_tol=0.0, abs_tol=tolerance):
    ...
    exchange_fee = expected_turnover * fee_bps / 10_000.0
    net = gross - exchange_fee
    _assert_series_close("turnover", persisted["turnover"], expected_turnover, tolerance=tolerance)
    ...
    delayed_rows_verified = 0
    for ordinal, fold_payload in enumerate(folds, start=1):
        fold_mapping = _mapping(fold_payload, f"fold {ordinal}")
        fold_id = _positive_integer(fold_mapping.get("fold"), f"fold {ordinal} identifier")
        ...
        if len(fold_frame) > 1:
            ...
            _assert_series_close(
                f"fold {fold_id} delayed position",
                fold_frame["position"].iloc[1:],
                expected_position,
                tolerance=tolerance,
            )
            delayed_rows_verified += len(fold_frame) - 1
    ...
    evaluation_start = _utc_timestamp(data_summary.get("evaluation_start"), "evaluation_start")
    evaluation_end = _utc_timestamp(data_summary.get("evaluation_end"), "evaluation_end")
    ...
    fold_boundaries = int(persisted["fold"].ne(persisted["fold"].shift()).sum() - 1)
    return {
        "status": "passed",
        ...
        "within_fold_delayed_position_rows_verified": delayed_rows_verified,
        "fold_boundary_position_transitions_verified": fold_boundaries,
