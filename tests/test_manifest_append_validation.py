...
def _entry() -> dict[str, object]:
    ...
    return build_experiment_manifest_entry(
        ...
        candidate_count=27,
        result_classification="fixture-only append validation; no performance claim",
...
def test_manifest_append_rejects_tampered_evidence_before_creating_file(tmp_path: Path) -> None:
    entry = _entry()
...
def test_manifest_append_rejects_noncanonical_existing_evidence(tmp_path: Path) -> None:
    entry = _entry()
    manifest = tmp_path / "experiment-manifest.jsonl"
