...
_ANALYSIS_PATH = _ROOT / "reports" / "research" / "omega-vs-volatility-benchmark" / "analysis.py"
_RESULT_PATH = _ROOT / "reports" / "research" / "omega-vs-volatility-benchmark" / "result.json"
...
_SPEC = importlib.util.spec_from_file_location("omega_benchmark_analysis", _ANALYSIS_PATH)
...
def test_omega_matches_independent_real_okx_calculation() -> None:
    values = _real_returns_frame()["benchmark_buy_and_hold_return"].to_numpy(dtype=float)
    gains = float(np.maximum(values, 0.0).sum())
    losses = float(-np.minimum(values, 0.0).sum())

    assert _ANALYSIS.omega_ratio(values) == pytest.approx(gains / losses)


def test_paired_omega_bootstrap_is_deterministic_and_preserves_rows() -> None:
    ...
    first = _ANALYSIS.bootstrap_omega_delta(strategy, benchmark, **kwargs)
    second = _ANALYSIS.bootstrap_omega_delta(strategy, benchmark, **kwargs)

    assert first == second
    assert first["observed_delta"] == pytest.approx(
        _ANALYSIS.omega_ratio(strategy) - _ANALYSIS.omega_ratio(benchmark)
    )
