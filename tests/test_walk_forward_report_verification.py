...
def test_persisted_walk_forward_verifier_reconstructs_path_diagnostics(
    ...
    assert (
        verification["evaluation_start"] == pd.Timestamp(returns["timestamp"].iloc[0]).isoformat()
    )
    assert verification["evaluation_end"] == pd.Timestamp(returns["timestamp"].iloc[-1]).isoformat()
