...
def test_live_trade_flow_source_schema_checkpoint() -> None:
    ...
    subprocess.run(
        [
            sys.executable,
            "scripts/stress_okx_trade_flow_chronology.py",
            "--output-dir",
            str(output_dir),
        ],
        check=True,
    ...
    assert chronology["verdict"] == (
        "trade_flow_numeric_id_chronology_proven_by_public_rest_sequence"
    )
    ...
    for market in chronology["markets"]:
        ...
        impact = market["tie_break_impact"]
        ...
        for page_name in ("older_page", "newer_page"):
            ...
            attack = page["order_mutation_attack"]
            assert attack["legacy_set_only_gate_false_pass_reproduced"] is True
            assert attack["sequence_gate_rejected_mutation"] is True
