...
def test_record_fails_closed_while_replay_snapshot_is_open(
    ...
    real_inventory = store_module._core._bounded_decision_names

    def pause_after_snapshot_list(directory_descriptor: int) -> tuple[str, ...]:
        names = real_inventory(directory_descriptor)
        replay_listed.set()
        if not release_replay.wait(timeout=5):
            raise RuntimeError("timed out waiting to release replay")
        return names

    monkeypatch.setattr(store_module._core, "_bounded_decision_names", pause_after_snapshot_list)
    ...
    monkeypatch.setattr(store_module._core, "_bounded_decision_names", real_inventory)
...
def test_store_lock_uses_the_pinned_directory_inode_when_legacy_path_is_replaced(
    tmp_path: Path,
) -> None:
    target_path = tmp_path / "target-intents.jsonl"
    decision_directory = tmp_path / "paper-decisions"
    ...
    first_descriptor = os.open(decision_directory, flags)
    second_descriptor = os.open(decision_directory, flags)
    legacy_lock_path = decision_directory / ".paper-decision-store.lock"
    try:
        with store_module._core._store_lock(first_descriptor):
            legacy_lock_path.write_bytes(b"replacement")
            legacy_lock_path.chmod(0o600)
            with (
                pytest.raises(RuntimeError, match="paper order decision lock is held"),
                store_module._core._store_lock(second_descriptor),
            ):
                pytest.fail("replacement pathname bypassed the store lock")
    finally:
