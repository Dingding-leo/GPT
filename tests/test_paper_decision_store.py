...
def test_record_size_limit_accepts_boundary_and_rejects_before_staging(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    exact_root = tmp_path / "exact"
    exact_root.mkdir()
    target_path, decision_directory, target = _paths(exact_root)
    decision = _decision(target)
    payload_size = len(decision.to_json_bytes())
    monkeypatch.setattr(core_module, "_MAX_DECISION_RECORD_BYTES", payload_size)

    assert record_paper_order_decision(target_path, decision_directory, decision) == decision

    overflow_root = tmp_path / "overflow"
    overflow_root.mkdir()
    overflow_target_path, overflow_directory, overflow_target = _paths(overflow_root)
    overflow_decision = _decision(overflow_target)
    assert len(overflow_decision.to_json_bytes()) == payload_size
    monkeypatch.setattr(core_module, "_MAX_DECISION_RECORD_BYTES", payload_size - 1)
    ...
    with pytest.raises(ValueError, match="exceeds the maximum record size"):
        record_paper_order_decision(
            overflow_target_path,
            overflow_directory,
            overflow_decision,
        )

    assert pending_target_position_intents(overflow_target_path, overflow_directory) == (
        overflow_target,
    )
    assert not list(overflow_directory.glob("*.json"))
