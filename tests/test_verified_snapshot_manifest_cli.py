...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            "29841895366",
            "provenance.source_workflow_run_id must be a positive integer",
        ),
        (
            "source_artifact_id",
            0,
            "provenance.source_artifact_id must be a positive integer",
        ),
        (
            "source_artifact_sha256",
            "not-a-digest",
            "provenance.source_artifact_sha256 must be a SHA-256 digest",
        ),
    ],
)
def test_loader_rejects_invalid_provenance_copied_by_builder(
    tmp_path: Path,
    btc_usdt_prices: pd.Series,
    field: str,
...
def test_manifest_guide_distinguishes_builder_and_loader_provenance_validation() -> None:
    guide = (_REPOSITORY_ROOT / "docs/VERIFIED_SNAPSHOT_MANIFEST.md").read_text(encoding="utf-8")

    assert "`raw_pages_sha256`（存在时）是格式有效的 SHA-256" in guide
    assert "经过 ISO-8601 与显式时区校验的 `fetched_at_utc`" in guide
    assert "metadata 中存在 `source_workflow_run_id` 声明" in guide
    assert "`source_workflow_run_id` 只检查字段是否存在" in guide
    assert "helper 当前不会校验这些字段的类型、取值或格式" in guide
    assert "`load_verified_price_snapshot()` 重新验证 manifest" in guide
    assert "`source_workflow_run_id` 和 `source_artifact_id`（存在时）必须是非布尔正整数" in guide
    assert "`source_artifact_sha256`（存在时）必须是格式有效的 SHA-256" in guide
    assert "`source_artifact_name` 与 `source_head_sha` 仍然只是未联网核验的声明" in guide
