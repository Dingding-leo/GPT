...
import run_prospective_simple_trend_shadow_20260803_1200 as checkpoint

PREVIOUS_DECISION_HOUR_MS = 1_785_751_200_000
REALIZED_DECISION_HOUR_MS = 1_785_754_800_000
PRIOR_REPORTED_SIGNAL_HOUR_MS = 1_785_754_800_000
LATEST_COMPLETE_SIGNAL_HOUR_MS = 1_785_758_400_000
RECENT_WINDOW_FIRST_DECISION_HOUR_MS = 1_785_740_400_000
RECENT_WINDOW_LAST_DECISION_HOUR_MS = REALIZED_DECISION_HOUR_MS
PAYOFF_END_OPEN_HOUR_MS = 1_785_762_000_000

PRIOR_RESULT_SHA256 = (
    "772695e624a1e7d4b82e54b0b4ea12c117505b864bc2f33f48e96989e0b633f4"
)
PRIOR_ARTIFACT_SHA256 = (
    "df9e3e8279d2249690005a9b284957127b4770b5b6d25e419c443f34c4f3bc16"
)
PRIOR_PULL_REQUEST = 1056
PRIOR_WORKFLOW_RUN = 30813553441
PRIOR_ARTIFACT_ID = 8855753488
PRIOR_CUMULATIVE_REALIZED_HOURS = 635
UPDATED_CUMULATIVE_REALIZED_HOURS = 636

LATEST_ARCHITECTURE: dict[str, Any] = {
    "family_id": "causal-own-price-jump-discontinuity-programme-closure-1h-v1",
    "issue": 1054,
    "pull_request": 1057,
    "exact_head": "80ff8f18694e3c6b2e12795a65b686029b460d24",
    "workflow_run": 30814886982,
    "artifact_id": 8856284130,
    "artifact_sha256": (
        "2cae6276bd8ee9da0504f6357be3fef6b9d523a0a6c17aa5e89445d69a6d7741"
    ),
    "candidate_count": 0,
    "parameter_grid_count": 0,
    "bound_mechanism_count": 2,
    "supportive_mechanism_count": 0,
    "fixed_targets": ["BTC-USDT", "ETH-USDT", "NEAR-USDT", "APT-USDT"],
    ...
    "highest_value_failure": (
        "Both the executable BTC/ETH bipower-jump candidate and the external "
        "NEAR/APT jump-share diagnostic failed bilateral transferability, "
        "dependence and breadth gates; every leave-one-mechanism-out subset "
        "retained zero admissible mechanisms."
    ),
    "next_action": (
        "Do not reopen completed own-price jump or discontinuity mechanisms."
    ),
}
...
def finalize(result: dict[str, Any]) -> dict[str, Any]:
    ...
    result["training_authorized_correction"] = {
        ...
        "reason": (
            "Issue 1054 terminally closed the completed jump/discontinuity "
            "programme with zero admissible mechanisms. Issue 1055 remains "
            "preregistered but unevaluated. Neither creates correction authority."
        ),
    }
    result["next_strategy_action"] = (
        ...
        "alter targets, source, features, penalty, threshold, cadence, segments, "
        "comparators, folds, bootstrap, delay, fee or acceptance gates after access."
    )
...
def validate(result: dict[str, Any]) -> None:
    ...
    assert window["prior_last_signal_bar_start"] == "2026-08-03T11:00:00Z"
    assert window["latest_complete_signal_bar_start"] == "2026-08-03T12:00:00Z"
    assert window["realized_payoff_interval_start"] == "2026-08-03T12:00:00Z"
    assert window["realized_payoff_interval_end"] == "2026-08-03T13:00:00Z"
    assert window["new_signal_bar_count"] == 1
    assert window["new_realized_payoff_intervals"] == 1
    assert window["prior_cumulative_realized_hours"] == 635
    assert window["updated_cumulative_realized_hours"] == 636
    ...
    assert machine["payoff_end_open_timestamp"] == "2026-08-03T13:00:00Z"
    assert machine["future_signal_accessed"] is False
    assert machine["strategy_value_changed"] is False
    assert machine["updated_cumulative_realized_hours"] == 636
...
def persist(output_dir: Path, result: dict[str, Any]) -> str:
    ...
    report = (
        "# Prospective simple-trend checkpoint through 13:00 UTC on "
        "3 August 2026\n\n"
