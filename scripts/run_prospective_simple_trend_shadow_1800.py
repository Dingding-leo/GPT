...
BAR = "1H"
PREVIOUS_DECISION_HOUR_MS = 1_785_250_800_000  # 2026-07-28T15:00:00Z
REALIZED_DECISION_HOUR_MS = 1_785_254_400_000  # 2026-07-28T16:00:00Z
PRIOR_REPORTED_SIGNAL_HOUR_MS = 1_785_258_000_000  # 2026-07-28T17:00:00Z
LATEST_COMPLETE_SIGNAL_HOUR_MS = 1_785_261_600_000  # 2026-07-28T18:00:00Z
PRIOR_RESULT_SHA256 = "114bfcdddee856aa73f782b9ac93d249276419d89a9b49d8cf578c7f753a98d4"
PRIOR_ARTIFACT_SHA256 = "91fffdbcf4d75c4b0603586e6d5396adf225c80ae29130c13588756681cdc8d5"
POLICY_SIGNATURE = (
...
def fetch_candles(
    ...
    earliest_required = PREVIOUS_DECISION_HOUR_MS - LOOKBACK_HOURS * HOUR_MS
    ...
    for page_index in range(20):
        ...
        for row in rows:
            ...
            if prior is not None and prior != record:
                when = iso_utc(timestamp_ms)
                raise ValueError(f"conflicting duplicate at {when}: {instrument}")
...
def validate_grid(
    ...
    first = PREVIOUS_DECISION_HOUR_MS - LOOKBACK_HOURS * HOUR_MS
    last = LATEST_COMPLETE_SIGNAL_HOUR_MS
    count = 0
    for timestamp_ms in range(first, last + HOUR_MS, HOUR_MS):
...
def calculate_market(
    ...
    margins = {
        ...
        for timestamp_ms in signal_hours
    }
    targets = {timestamp_ms: int(value > 0.0) for timestamp_ms, value in margins.items()}

    previous_position = targets[PREVIOUS_DECISION_HOUR_MS]
    position = targets[REALIZED_DECISION_HOUR_MS]
    turnover = abs(position - previous_position)
    fee = turnover * FEE_RATE
    payoff_open_start = REALIZED_DECISION_HOUR_MS + HOUR_MS
    ...
    latest_margin = margins[LATEST_COMPLETE_SIGNAL_HOUR_MS]
    latest_target = targets[LATEST_COMPLETE_SIGNAL_HOUR_MS]
    prior_margin = margins[PRIOR_REPORTED_SIGNAL_HOUR_MS]
    prior_target = targets[PRIOR_REPORTED_SIGNAL_HOUR_MS]
    new_decision = {
        "signal_hour_start_ms": LATEST_COMPLETE_SIGNAL_HOUR_MS,
        "signal_hour_start": iso_utc(LATEST_COMPLETE_SIGNAL_HOUR_MS),
        "signal_available_at": iso_utc(LATEST_COMPLETE_SIGNAL_HOUR_MS + HOUR_MS),
        "margin": latest_margin,
        "target": latest_target,
        "execution_open": iso_utc(LATEST_COMPLETE_SIGNAL_HOUR_MS + HOUR_MS),
    ...
    return {
        ...
        "new_signal_observations": 1,
        "new_decisions": [new_decision],
        "new_long_targets": latest_target,
        "no_trade_frequency": float(latest_target == 0),
        "pending_target_changes": int(latest_target != prior_target),
        "realized_interval": {
            "decision_hour_start_ms": REALIZED_DECISION_HOUR_MS,
            "decision_hour_start": iso_utc(REALIZED_DECISION_HOUR_MS),
            "signal_available_at": iso_utc(REALIZED_DECISION_HOUR_MS + HOUR_MS),
        ...
        "signal_drift": {
            "margin_at_prior_reported_signal_hour": prior_margin,
            "margin_at_latest_complete_signal_hour": latest_margin,
            "margin_change": latest_margin - prior_margin,
            "target_changed_since_prior_reported_signal_hour": latest_target != prior_target,
        },
        "sharpe": None,
...
def write_outputs(output_dir: Path, result: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        "- Newly realized payoff intervals: `1`",
        "",
        "| Market | Position | Net return | Turnover | Fee | Latest margin |",
        "|---|---:|---:|---:|---:|---:|",
    ]
...
def run(output_dir: Path, base_url: str) -> dict[str, Any]:
    ...
    result = {
        ...
        "prospective_lineage": {
            ...
            "prior_last_signal_bar_start": iso_utc(PRIOR_REPORTED_SIGNAL_HOUR_MS),
        ...
        "window": {
            ...
            "prior_cumulative_realized_hours": 496,
            "updated_cumulative_realized_hours": 497,
        },
