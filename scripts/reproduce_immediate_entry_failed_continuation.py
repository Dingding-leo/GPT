# ruff: noqa
# fmt: off
"""Reproduce issue #676 from immutable public OKX 1H candles."""
...
FEE=5e-4; ANN=8760.; W=2160; GRACE=168; N=43441; FOLD=2160
...
def positions(d:pd.DataFrame):
    n=len(d); close=d.close.to_numpy(float)
    p={k:np.zeros(n-1) for k in ("candidate","b0","b1")}
    c=b0=b1=0.; prev_daily_base=False; onset=None; onset_close=None; locked=False
    rows=[]; regimes=[]; active=None
    for t in range(W,n-1):
        base=bool(close[t]>close[t-W]); b0=float(base)
        if d.index[t].hour==0:
            prev_c=c; event="hold"; regime_ret=None; recent_ret=None; failed=False
            onset_flag=base and not prev_daily_base
            if not base:
                if active is not None:
                    active["end_decision"]=t; active["base_exit"]=True; regimes.append(active); active=None
                c=0.; onset=None; onset_close=None; locked=False; event="base_exit" if prev_c>0 else "base_nonpositive"
            elif onset_flag:
                if active is not None: raise ValueError("overlapping regime")
                onset=t; onset_close=close[t]; locked=False; c=1.; event="onset_entry"
                active={"onset_decision":t,"onset_execution":t+1,"onset_close":float(onset_close),"failed_exit_decision":None,"failed_exit_execution":None,"base_exit":False,"end_decision":None}
            else:
                if onset is None or active is None: raise ValueError("positive regime missing onset")
                age=t-onset
                if locked:
                    c=0.; event="locked_out"
                else:
                    if age>=GRACE:
                        regime_ret=float(math.log(close[t]/onset_close)); recent_ret=float(math.log(close[t]/close[t-GRACE]))
                        failed=bool(regime_ret<=0 and recent_ret<0)
                    if failed:
                        c=0.; locked=True; event="failed_exit"
                        active["failed_exit_decision"]=t; active["failed_exit_execution"]=t+1; active["failed_regime_return"]=regime_ret; active["failed_recent_return"]=recent_ret
                    else:
                        c=1.; event="hold_long"
            b1=float(base)
            rows.append({"decision":t,"execution":t+1,"base":base,"prev_daily_base":prev_daily_base,"onset":onset_flag,"onset_decision":onset,"age_hours":None if onset is None else t-onset,"regime_return":regime_ret,"recent_return":recent_ret,"failed":failed,"locked":locked,"prev_candidate":prev_c,"candidate":c,"b1":b1,"event":event})
...
def bootstrap(c,b):
    c=c[OOS[0]:OOS[1]]; b=b[OOS[0]:OOS[1]]; n=len(c); rng=np.random.default_rng(SEED); md=np.empty(5000); sd=np.empty(5000); off=np.arange(GRACE); blocks=math.ceil(n/GRACE)
    for q in range(0,5000,100):
        idx=(rng.integers(0,n-GRACE+1,size=(100,blocks))[:,:,None]+off).reshape(100,-1)[:,:n]; cs=c[idx]; bs=b[idx]; cm=cs.mean(1); bm=bs.mean(1); cv=cs.std(1,ddof=1); bv=bs.std(1,ddof=1)
...
def span_overlap(start,stop,span):
    return max(start,span[0]),min(stop,span[1])
...
def diagnostics(d,rows,regimes,p,a):
    ...
    exit_rows=[r for r in rows if r["event"]=="failed_exit"]
    def fwd_stats(h):
        vals=[]
        for r in exit_rows:
            x=r["execution"]
    ...
    for rg in regimes:
        start=rg["onset_execution"]; stop=min(rg["end_decision"]+1,len(p["candidate"]))
        os,oe=span_overlap(start,stop,OOS)
        if oe<=os: continue
    ...
    for rg in regimes:
        if rg["failed_exit_execution"] is not None:
            stop=min(rg["end_decision"]+1,len(lockmask)); lockmask[rg["failed_exit_execution"]:stop]=1
    ...
    return dict(oos_failed_exits=len(failed_oos),failed_exit_records=[{k:r[k] for k in ("decision","execution","age_hours","regime_return","recent_return")} for r in failed_oos],forward_after_failed_exit_24h=fwd_stats(24),forward_after_failed_exit_168h=fwd_stats(168),oos_event_counts=event_counts,oos_regimes=regime_stats,oos_regimes_overlapping=len(regime_stats),oos_regimes_started=started,oos_regime_outcomes_vs_b1=outcomes,oos_failed_exit_regime_outcomes_vs_b1=failed_outcomes,b1_only_exposure_hours=float(b1_only.sum()),candidate_only_exposure_hours=float(candidate_only.sum()),b1_only_market_return=float((b1_only*m).sum()),candidate_only_market_return=float((candidate_only*m).sum()),exposure_delta_market_arithmetic_return=exposure,incremental_fees_candidate_minus_b1=fee,observed_arithmetic_net_delta=observed,candidate_transition_turnover=transitions,lockout_attribution_identity_passes=True,decomposition_identity_passes=True)
...
def protocol():
    return {"family_id":"immediate-entry-failed-continuation-exit-1h-v1","issue":676,"research_parent":"5a0fcc97d1a882f8223656c51f5bb8055f534e38","bar":"1H","canonical_fee_one_way":FEE,"candidate_count":1,"parameter_grid_count":0,"sources":{m:{"artifact_id":ART[m],"csv_sha256":HASH[m],"provider":"OKX public confirmed SPOT"} for m in HASH},"sample":{"parsed_prefix_bars":N,"warmup":[0,2880],"training":list(TRAIN),"development_oos":list(OOS),"full_scored":list(FULL),"folds":12,"fold_hours":FOLD,"later_suffix_unread":True},"feature":{"base":"close_t > close_(t-2160)","onset":"first positive daily base decision after a non-positive daily base decision","grace_hours":GRACE,"regime_return":"log(close_t/onset_close)","recent_return":"log(close_t/close_(t-168))","failed":"regime_return <= 0 and recent_return < 0"},"policy":{"decision_cadence":"daily completed 00:00 UTC","execution":"next hourly open","entry":"immediate at every new positive base-trend onset","grace":"remain long for first 168 completed hours","failed_exit":"first failed state after grace","reentry":"only after a later distinct onset following non-positive base","base_exit":"immediate cash when base non-positive","exposure_states":[0,1],"fees":"5 bps per absolute exposure change"},"uncertainty":{"resamples":5000,"block_hours":168,"paired_non_circular":True,"seed":SEED}}
...
def main():
    ...
    result={"protocol":protocol(),"markets":markets,"bilateral_accepted":accepted,"verdict":"accept_immediate_entry_failed_continuation_exit_family" if accepted else "reject_exact_immediate_entry_failed_continuation_exit_family","repaired_discrepancy":"The initial regime diagnostic reported only a strict-improvement count over all OOS-overlapping regimes, which conflated tied regimes with failures and did not distinguish the one BTC regime that began before OOS. The terminal diagnostic now reports started versus overlapping regimes and improved/tied/worse outcomes for all regimes and failed-exit regimes separately. No signal, position, fee, performance metric, uncertainty result, gate or verdict changed.","remaining_blocker":"the joint failed-continuation condition does not yet show bilateral fold breadth and uncertainty-supported improvement over immediate slow-trend participation.","next_experiment":"Preregister one own-history-only trend-onset loss-budget architecture that enters immediately, never delays entry, and permits one irreversible same-regime exit only when cumulative adverse excursion from the highest completed daily close since onset exceeds the trailing 720H robust volatility scale while regime return is non-positive; one candidate, no grid and no re-entry until a new onset."}
    text=json.dumps(result,indent=2,sort_keys=True,allow_nan=False)+"\n"; (z.out/"result.json").write_text(text); (z.out/"protocol.json").write_text(json.dumps(protocol(),indent=2,sort_keys=True)+"\n"); print(text)
