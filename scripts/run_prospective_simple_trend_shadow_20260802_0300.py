...
def write_outputs(output_dir: Path, result: dict[str, Any]) -> None:
    ...
    lines = [
        "# Prospective simple-trend shadow through the 03:00 UTC payoff boundary on 2 August 2026",
        "",
    ...
    lines.extend(
        [
            ...
            "",
            "Issue #943 / PR #946 remains the latest terminal strategy-family result. The technically valid public mark/index panel did not reopen the rejected derivatives-crowding family, and no replacement architecture is active.",
            "",
            "No training-authorised correction, canonical mutation, observation-epoch restart, paper-trading authority or live-trading authority is permitted.",
            "",
            "```json",
...
def run(output_dir: Path, base_url: str) -> dict[str, Any]:
    ...
    result["checkpoint_type"] = "complete_signal_and_realized_payoff"
    ...
    result["window"]["realized_payoff_interval_start"] = realised[
        "payoff_open_start"
    ]
    result["window"]["realized_payoff_interval_end"] = realised[
        "payoff_open_end"
    ]
    ...
    result["aggregate_forward_scorecard"] = {
        ...
        "no_trade_frequency": None
        if signal_frequency is None
        else 1.0 - signal_frequency,
    }
    ...
    result["training_authorized_correction"] = {
        ...
        "reason": (
            "the latest admissibility closure rejected reopening the mark/index basis family, "
            "and no materially orthogonal strategy architecture is preregistered"
        ),
    }
    exposed = any(
    ...
    result["machine_readable_verdict"] = {
        ...
        "signal_frequency": result["aggregate_forward_scorecard"][
            "signal_frequency"
        ],
