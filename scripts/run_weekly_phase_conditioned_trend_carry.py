...
def fit_model(frame: pd.DataFrame) -> dict[str, object]:
    ...
    hour_of_week = (
        execution_timestamps.dt.weekday.to_numpy() * 24
        + execution_timestamps.dt.hour.to_numpy()
    ).astype(int)
    matrix = design_matrix(hour_of_week, CONFIG["fourier_harmonics"])
    coefficients = np.linalg.lstsq(matrix, target, rcond=None)[0]
    profile = design_matrix(
        np.arange(168), CONFIG["fourier_harmonics"]
    ) @ coefficients
...
def candidate_positions(
    frame: pd.DataFrame, favourable_weekdays: list[int]
) -> np.ndarray:
    length = len(frame) - 2
    close = frame["close"].to_numpy(float)
    positions = np.zeros(length)
...
def net_series(
    positions: np.ndarray, gross_returns: np.ndarray
) -> tuple[np.ndarray, np.ndarray]:
    changes = np.abs(np.diff(np.r_[0.0, positions]))
    net_returns = (
        positions * gross_returns - CONFIG["fee_one_way"] * changes
    )
...
def annualised_sharpe(values: np.ndarray) -> float | None:
    ...
    if len(series) < 2 or standard_deviation == 0 or not np.isfinite(
        standard_deviation
    ):
        return None
...
def breadth(
    ...
    for fold_index in range(CONFIG["fold_count"]):
        ...
        metrics = segment_metrics(
            net_returns, positions, changes, fold_start, fold_end
        )
        fold_returns.append(metrics["net_return"])

    positive_folds = [value for value in fold_returns if value > 0]
    concentration = (
        max(positive_folds) / sum(positive_folds) if positive_folds else None
    )
    execution_timestamps = frame["timestamp"].iloc[
        1 : len(net_returns) + 1
...
def paired_bootstrap(
    ...
    for resample_index in range(len(mean_deltas)):
        starts = rng.integers(
            0, sample_size - block_size + 1, size=block_count
        )
        indices = np.concatenate(
            [np.arange(value, value + block_size) for value in starts]
        )[:sample_size]
        candidate_sample = candidate_oos[indices]
        benchmark_sample = benchmark_oos[indices]
        mean_deltas[resample_index] = (
        ...
        sharpe_deltas[resample_index] = (
            0.0 if candidate_sharpe is None else candidate_sharpe
    ...
    return {
        ...
        "sharpe_delta_quantiles": [
            float(value) for value in np.quantile(sharpe_deltas, quantiles)
        ],
    }
...
def phase_persistence(
    frame: pd.DataFrame, model: dict[str, object]
) -> dict[str, object]:
    close = frame["close"].to_numpy(float)
    open_price = frame["open"].to_numpy(float)
    start, end = CONFIG["oos"]
    ...
    means = np.array(
        [
            np.mean(weekday_values[weekday])
            if weekday_values[weekday]
            else np.nan
            for weekday in range(7)
        ]
    )
    ...
    pearson = (
        float(np.corrcoef(scores[valid], means[valid])[0, 1])
        if valid.sum() > 1
        else None
    )
    spearman = (
        float(
            np.corrcoef(
                rank_values(scores[valid]), rank_values(means[valid])
            )[0, 1]
        )
        if valid.sum() > 1
        else None
    )
    realised_order = sorted(
        range(7), key=lambda weekday: (-means[weekday], weekday)
    )
    frozen = set(model["favourable_weekday_indices"])
    realised_top = set(realised_order[:2])
    return {
        "oos_positive_trend_daily_support": [
            len(weekday_values[weekday]) for weekday in range(7)
        ],
        ...
        "frozen_selected_mean": float(
            np.nanmean([means[value] for value in frozen])
        ),
        "unselected_mean": float(
            np.nanmean(
                [means[value] for value in range(7) if value not in frozen]
            )
        ),
    }
...
def policy_metrics(
    ...
    metrics = {
        segment: segment_metrics(
            net_returns, positions, changes, *CONFIG[segment]
        )
...
def acceptance_gates(
    ...
    return {
        ...
        "profitable_folds_at_least_7_of_12": (
            breadth_result["profitable_folds"] >= 7
        ),
        "profitable_year_segments_at_least_3": (
            breadth_result["profitable_years"] >= 3
        ),
...
def run(btc_csv: str, eth_csv: str) -> dict[str, object]:
    ...
    for instrument, (path, expected_hash, artifact_id) in sources.items():
        ...
        candidate_position = candidate_positions(
            frame, model["favourable_weekday_indices"]
        )
        b0_position = trend_positions(frame, daily=False)
        b1_position = trend_positions(frame, daily=True)
        candidate_net, candidate_changes = net_series(
            candidate_position, gross_returns
        )
        ...
        policies = {
            "candidate": policy_metrics(
                candidate_net, candidate_position, candidate_changes
            ),
            "b0_hourly_2160h_trend": policy_metrics(
                b0_net, b0_position, b0_changes
            ),
            "b1_daily_2160h_trend": policy_metrics(
                b1_net, b1_position, b1_changes
            ),
        }
        breadth_result = breadth(
            frame, candidate_net, candidate_position, candidate_changes
        )
        residual_b0 = residual_sharpe(
            candidate_net, b0_net, *CONFIG["oos"]
        )
        residual_b1 = residual_sharpe(
            candidate_net, b1_net, *CONFIG["oos"]
        )
...
def main() -> None:
    ...
    summary = {
        ...
        "BTC-USDT": result["markets"]["BTC-USDT"]["policies"]["candidate"][
            "oos"
        ],
        "ETH-USDT": result["markets"]["ETH-USDT"]["policies"]["candidate"][
            "oos"
        ],
    }
