...
import run_prospective_simple_trend_shadow_20260803_1100 as checkpoint

PREVIOUS_DECISION_HOUR_MS = 1_785_747_600_000
REALIZED_DECISION_HOUR_MS = 1_785_751_200_000
PRIOR_REPORTED_SIGNAL_HOUR_MS = 1_785_751_200_000
LATEST_COMPLETE_SIGNAL_HOUR_MS = 1_785_754_800_000
RECENT_WINDOW_FIRST_DECISION_HOUR_MS = 1_785_736_800_000
RECENT_WINDOW_LAST_DECISION_HOUR_MS = REALIZED_DECISION_HOUR_MS
PAYOFF_END_OPEN_HOUR_MS = 1_785_758_400_000

PRIOR_RESULT_SHA256 = (
    "c207d90f441f66b99d5547ba5048ff45707841f9fd5be7b3f262eecc2bb4ba45"
)
PRIOR_ARTIFACT_SHA256 = (
    "06189efc998b2930782dd8791456a9d1c425b25613609425415182c03f82742d"
)
PRIOR_PULL_REQUEST = 1052
PRIOR_WORKFLOW_RUN = 30809613662
PRIOR_ARTIFACT_ID = 8854202316
PRIOR_CUMULATIVE_REALIZED_HOURS = 634
UPDATED_CUMULATIVE_REALIZED_HOURS = 635
...
LATEST_ARCHITECTURE: dict[str, Any] = {
    ...
    "status": "terminally_rejected_before_oos",
    "verdict": (
        "reject_causal_own_price_bipower_jump_share_contraction_"
        "information_premise_1h_v1"
    ),
    "highest_value_failure": (
        "NEAR contradicted the frozen continuation sign. APT supplied only a "
        "support-deficient, dependence-unsupported point effect with adverse-"
        "excursion and concentration failures."
    ),
    "next_action": (
        "Do not rescue jump-share contraction through sign reversal, alternate "
        "windows, thresholds, target substitution, pooling or single-market "
        "promotion."
    ),
}
...
ACTIVE_ARCHITECTURE: dict[str, Any] = {
    ...
    "paper_trading_authorized": False,
    "live_trading_authorized": False,
    "overlapping_open_closure_issue": 1054,
    "overlap_disposition": (
        "Neither open issue has correction authority; no mutation is permitted "
        "unless the frozen bilateral ridge protocol passes."
    ),
}
...
def finalize(result: dict[str, Any]) -> dict[str, Any]:
    ...
    result["training_authorized_correction"] = {
        ...
        "reason": (
            "Issue 1051 terminally rejected the frozen jump-share contraction "
            "premise before OOS. Issue 1055 is preregistered but unevaluated; "
            "issue 1054 is an overlapping open zero-candidate family closure. "
            "None creates correction authority."
        ),
    }
    result["next_strategy_action"] = (
        "Advance the unchanged independent BTC-USDT and ETH-USDT E2160 shadow "
        "at the next complete public 1H observation. Evaluate issue 1055 only "
        "under its frozen ETC-USDT/FIL-USDT ridge lag-strip validation-first "
        "protocol. Do not open OOS unless both validation arms pass, and do not "
        "alter markets, features, penalty, threshold, fee, cadence, segments, "
        "comparators, folds, bootstrap, delay or acceptance gates after access."
    )
...
def validate(result: dict[str, Any]) -> None:
    ...
    assert window["prior_last_signal_bar_start"] == "2026-08-03T10:00:00Z"
    assert window["latest_complete_signal_bar_start"] == "2026-08-03T11:00:00Z"
    assert window["realized_payoff_interval_start"] == "2026-08-03T11:00:00Z"
    assert window["realized_payoff_interval_end"] == "2026-08-03T12:00:00Z"
    assert window["new_signal_bar_count"] == 1
    assert window["new_realized_payoff_intervals"] == 1
    assert window["prior_cumulative_realized_hours"] == 634
    assert window["updated_cumulative_realized_hours"] == 635
    ...
    assert machine["payoff_end_open_timestamp"] == "2026-08-03T12:00:00Z"
    assert machine["future_signal_accessed"] is False
    assert machine["strategy_value_changed"] is False
    assert machine["updated_cumulative_realized_hours"] == 635
...
def persist(output_dir: Path, result: dict[str, Any]) -> str:
    ...
    report = (
        "# Prospective simple-trend checkpoint through 12:00 UTC on "
        "3 August 2026\n\n"
        "The 11:00 signal bar was provider-confirmed. The 12:00 candle supplied "
        "only its fixed opening price as the payoff endpoint.\n\n"
        "The NEAR/APT bipower jump-share contraction premise is terminally "
        "rejected before OOS. The ETC/FIL ridge lag-strip architecture in issue "
        "1055 is preregistered but unevaluated and cannot authorise a correction. "
        "Open closure issue 1054 also has zero correction authority.\n\n"
        "```json\n"
