...
import run_prospective_simple_trend_shadow_20260803_0900 as checkpoint

PREVIOUS_DECISION_HOUR_MS = 1_785_740_400_000
REALIZED_DECISION_HOUR_MS = 1_785_744_000_000
PRIOR_REPORTED_SIGNAL_HOUR_MS = 1_785_744_000_000
LATEST_COMPLETE_SIGNAL_HOUR_MS = 1_785_747_600_000
RECENT_WINDOW_FIRST_DECISION_HOUR_MS = 1_785_729_600_000
RECENT_WINDOW_LAST_DECISION_HOUR_MS = REALIZED_DECISION_HOUR_MS
PAYOFF_END_OPEN_HOUR_MS = 1_785_751_200_000

PRIOR_RESULT_SHA256 = (
    "590b498ee00e89cc08e2d74d66996e9d355567da33d7daa0c0b4c4fcc6847b54"
)
PRIOR_ARTIFACT_SHA256 = (
    "1e6b7d44907bf4de0b021a5debca483ceb58b45bcbc305a6741a24a2c288e3d6"
)
PRIOR_PULL_REQUEST = 1044
PRIOR_WORKFLOW_RUN = 30801197079
PRIOR_ARTIFACT_ID = 8850849130
PRIOR_CUMULATIVE_REALIZED_HOURS = 632
UPDATED_CUMULATIVE_REALIZED_HOURS = 633

LATEST_ARCHITECTURE: dict[str, Any] = {
    "family_id": "causal-own-price-topological-path-structure-programme-closure-1h-v1",
    "issue": 1046,
    "pull_request": None,
    "candidate_count": 0,
    "parameter_grid_count": 0,
    "bound_group_count": 2,
    "targets_admissible": 0,
    "groups_admissible": 0,
    "cohorts_admissible": 0,
    ...
    "verdict": (
        "reject_reopening_completed_own_price_topological_path_structure_"
        "mechanisms_1h_v1"
    ),
    "highest_value_failure": (
        "Neither recurrence determinism nor horizontal visibility produced "
        "bilateral positive continuation and downside information with broad "
        "fold transport, positive dependence lower bounds and stable delay effects."
    ),
    "next_action": (
        "Do not reopen completed recurrence or visibility mechanisms through "
        "alternate graph, radius, entropy, sign, cohort or favourable-period choices."
    ),
}
...
ACTIVE_ARCHITECTURE: dict[str, Any] = {
    ...
    "fixed_targets": ["AVAX-USDT", "DOT-USDT"],
    "candidate_count": 0,
    "parameter_grid_count": 0,
    "target_returns_accessed": False,
    "strategy_performance_accessed": False,
...
def finalize(result: dict[str, Any]) -> dict[str, Any]:
    ...
    result["training_authorized_correction"] = {
        ...
        "reason": (
            "Issue 1046 terminally rejected both completed topological path "
            "mechanisms. Issue 1047 is preregistered but unevaluated and therefore "
            "creates no training-authorized correction authority."
        ),
    }
    result["next_strategy_action"] = (
        "Advance the unchanged independent BTC-USDT and ETH-USDT E2160 shadow at "
        "the next complete public 1H observation. Evaluate issue 1047 only under "
        "its frozen AVAX-USDT/DOT-USDT nonparametric drawdown-recovery-hazard "
        "training protocol. Do not reopen recurrence or horizontal-visibility "
        "families, alter episode semantics, hazard smoothing, support floors, "
        "targets, chronology, fee, folds, bootstrap, delay or promotion rules "
        "after source or label access."
    )
...
def validate(result: dict[str, Any]) -> None:
    ...
    assert window["prior_last_signal_bar_start"] == "2026-08-03T08:00:00Z"
    assert window["latest_complete_signal_bar_start"] == "2026-08-03T09:00:00Z"
    assert window["realized_payoff_interval_start"] == "2026-08-03T09:00:00Z"
    assert window["realized_payoff_interval_end"] == "2026-08-03T10:00:00Z"
    assert window["new_signal_bar_count"] == 1
    assert window["new_realized_payoff_intervals"] == 1
    assert window["prior_cumulative_realized_hours"] == 632
    assert window["updated_cumulative_realized_hours"] == 633
    ...
    assert machine["payoff_end_open_timestamp"] == "2026-08-03T10:00:00Z"
    assert machine["future_signal_accessed"] is False
    assert machine["strategy_value_changed"] is False
    assert machine["updated_cumulative_realized_hours"] == 633
...
def persist(output_dir: Path, result: dict[str, Any]) -> str:
    ...
    report = (
        "# Prospective simple-trend checkpoint through 10:00 UTC on 3 August 2026\n\n"
        "The 09:00 signal bar was provider-confirmed. The 10:00 candle supplied "
        "only its fixed opening price as the payoff endpoint.\n\n"
        "The completed topological path-structure programme remains terminally "
        "rejected. The drawdown-recovery-hazard architecture in issue 1047 is "
        "preregistered but unevaluated and cannot authorise a correction.\n\n"
        "```json\n"
