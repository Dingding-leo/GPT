...
def positions(d: pd.DataFrame) -> dict[str, np.ndarray]:
    c, h, low = (d.close.to_numpy(float), d.high.to_numpy(float), d.low.to_numpy(float))
    n = len(d)
    ...
    for t in range(ENTRY, n - 1):
        ...
        if d.index[t].hour == 0:
            en = float(np.max(h[t - ENTRY:t]))
            ex = float(np.min(low[t - EXIT:t]))
            old = q
            if q == 0 and c[t] > en:
                q = 1.0
            elif q == 1 and c[t] < ex:
                q = 0.0
            b1 = b0
            if old != q and (not (q == 1 and c[t] > en or (q == 0 and c[t] < ex))):
...
def pack(d: pd.DataFrame, p: np.ndarray):
    ...
    m = o[1:] / o[:-1] - 1
    tr = np.r_[abs(p[0]), np.abs(np.diff(p))]
    g = p * m
    f = FEE * tr
    return (g - f, f, tr, g, m)
...
def boot(c, b):
    ...
    off = np.arange(168)
    for i in range(5000):
        st = rng.integers(0, n - 167, size=math.ceil(n / 168))
        ix = (st[:, None] + off).ravel()[:n]
        cr, br = (c[ix], b[ix])
        md[i] = ANN * np.mean(cr - br)
        sd[i] = (shp(cr) or 0) - (shp(br) or 0)
    return {'annualized_mean_delta': {'point': float(ANN * np.mean(c - b)), 'lower_95': float(np.quantile(md, 0.025)), 'upper_95': float(np.quantile(md, 0.975))}, 'sharpe_delta': {'point': float((shp(c) or 0) - (shp(b) or 0)), 'lower_95': float(np.quantile(sd, 0.025)), 'upper_95': float(np.quantile(sd, 0.975))}, 'block_hours': 168, 'resamples': 5000, 'seed': 20260729}

def episodes(pos, net, ts):
    a, z = OOS
    p = pos[a:z]
    x = net[a:z]
    bd = np.r_[0, np.flatnonzero(np.diff(p) != 0) + 1, len(p)]
    ep = []
    for s, e in zip(bd[:-1], bd[1:], strict=True):
        if p[s] == 1:
            ep.append({'start': str(ts[a + s]), 'stop': str(ts[a + e]), 'hours': int(e - s), 'net_return': float(np.prod(1 + x[s:e]) - 1)})
    r = [e['net_return'] for e in ep]
    return {'overlapping_oos_episodes': len(ep), 'episodes': ep, 'median_duration_hours': float(np.median([e['hours'] for e in ep])) if ep else None, 'profitable_episode_ratio': float(np.mean(np.array(r) > 0)) if ep else None, 'worst_episode_return': min(r) if ep else None}

def diag(d, pos, pk):
    a, z = OOS
    c, b = (pos['candidate'][a:z], pos['b1'][a:z])
    m = pk['candidate'][4][a:z]
    co = (c == 1) & (b == 0)
    bo = (c == 0) & (b == 1)
    daily_tr = np.array([t for t in range(*TRAIN) if d.index[t].hour == 0])
    daily_o = np.array([t for t in range(*OOS) if d.index[t].hour == 0])
    cp = pos['candidate']

    def trans(ix):
        x = cp[ix + 1]
        old = np.r_[cp[ix[0]], x[:-1]]
        return (int(((x == 1) & (old == 0)).sum()), int(((x == 0) & (old == 1)).sum()))
...
def main():
    ...
    q = ap.parse_args()
    frames = {'BTC-USDT': load(q.btc_csv, 'BTC-USDT'), 'ETH-USDT': load(q.eth_csv, 'ETH-USDT')}
    R = {'family_id': 'causal-range-breakout-hysteresis-trend-1h-v1', 'issue': 630, 'candidate_count': 1, 'parameter_grid_count': 0, 'canonical_fee_one_way': FEE, 'sample': {'training': list(TRAIN), 'development_oos': list(OOS), 'full_scored': list(FULL), 'fold_count': 12, 'fold_hours': FOLD, 'parsed_prefix_bars': PREFIX, 'later_suffix_unread': True}, 'sources': {n: {'workflow_run_id': 30401519824, 'artifact_id': ART[n], 'csv_sha256': HASH[n], 'source_total_observations': 43941} for n in HASH}, 'markets': {}}
    accepted = True
    for n, d in frames.items():
        ps = positions(d)
        pk = {k: pack(d, v) for k, v in ps.items()}
        spans = (('training', TRAIN), ('development_oos', OOS), ('full_scored', FULL))
        mm = {s: {k: met(pk[k], ps[k], sp) for k in ps} for s, sp in spans}
        cn, b0, b1 = (pk['candidate'][0], pk['b0'][0], pk['b1'][0])
        br = breadth(cn, d.index)
        bt = boot(cn, b1)
        rb0 = shp(cn[OOS[0]:OOS[1]] - b0[OOS[0]:OOS[1]])
        rb1 = shp(cn[OOS[0]:OOS[1]] - b1[OOS[0]:OOS[1]])
        c, b = (mm['development_oos']['candidate'], mm['development_oos']['b1'])
        gates = {'positive_net_return': c['net_return'] > 0, 'positive_sharpe': c['sharpe'] is not None and c['sharpe'] > 0, 'profitable_folds_at_least_7_of_12': br['profitable_folds'] >= 7, 'profitable_years_at_least_3': br['profitable_years'] >= 3, 'positive_fold_concentration_at_most_50pct': br['positive_fold_concentration'] <= 0.5, 'max_drawdown_within_2pp_of_b1': c['max_drawdown'] >= b['max_drawdown'] - 0.02, 'turnover_no_greater_than_b1': c['turnover'] <= b['turnover'], 'positive_edge_per_turnover': c['edge_per_turnover_bps'] > 0, 'edge_per_turnover_no_worse_than_b1': c['edge_per_turnover_bps'] >= b['edge_per_turnover_bps'], 'net_return_no_worse_than_b1': c['net_return'] >= b['net_return'], 'sharpe_no_worse_than_b1': c['sharpe'] >= b['sharpe'], 'positive_residual_sharpe_vs_b1': rb1 is not None and rb1 > 0, 'bootstrap_mean_delta_lower_bound_positive': bt['annualized_mean_delta']['lower_95'] > 0, 'bootstrap_sharpe_delta_lower_bound_positive': bt['sharpe_delta']['lower_95'] > 0, 'source_chronology_timing_fee_checks': True}
        ok = all(gates.values())
        accepted &= ok
        R['markets'][n] = {'metrics': mm, 'breadth': br, 'residual_sharpe_vs_b0': rb0, 'residual_sharpe_vs_b1': rb1, 'bootstrap_vs_b1': bt, 'diagnostics': diag(d, ps, pk), 'acceptance_gates': gates, 'accepted': ok}
    R['accepted'] = accepted
    R['verdict'] = 'accept_for_g1_nomination' if accepted else 'reject_exact_causal_range_breakout_hysteresis_trend_family'
    q.output.parent.mkdir(parents=True, exist_ok=True)
    q.output.write_text(json.dumps(R, indent=2, sort_keys=True) + '\n')
