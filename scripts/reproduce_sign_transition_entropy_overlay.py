...
def sharpe(x):
    sd = np.std(x, ddof=1)
    return None if not np.isfinite(sd) or sd <= 0 else float(math.sqrt(ANN) * np.mean(x) / sd)
...
def breadth(net, index):
    folds = [float(np.prod(1 + net[OOS[0] + k * FOLD : OOS[0] + (k + 1) * FOLD]) - 1) for k in range(12)]
...
def bootstrap(c, b):
    ...
    for z in range(0, RESAMPLES, 100):
        ...
        md[z : z + q] = ANN * (cm - bm)
        sd[z : z + q] = np.divide(math.sqrt(ANN) * cm, cs, out=np.zeros(q), where=cs > 0) - np.divide(math.sqrt(ANN) * bm, bs, out=np.zeros(q), where=bs > 0)
    return {
        "annualized_mean_delta": {"point": float(ANN * np.mean(c - b)), "lower_95": float(np.quantile(md, .025)), "upper_95": float(np.quantile(md, .975))},
        "sharpe_delta": {"point": float((sharpe(c) or 0) - (sharpe(b) or 0)), "lower_95": float(np.quantile(sd, .025)), "upper_95": float(np.quantile(sd, .975))},
    }
...
def diagnose(p, arrays, events):
    ...
    def fwd(rows):
        ...
        for x in rows:
            k = x[0]
            vals.append(float(np.prod(1 + market[k:min(k + RW, len(market))]) - 1))
        return {"count": len(rows), "mean_next_168h": float(np.mean(vals)) if vals else None, "positive_next_168h_share": float(np.mean(np.array(vals) > 0)) if vals else None}
    half = c[i:j] == .5
    ...
    return {
        "raw_risk_triggers": len(risk), "raw_recovery_triggers": len(recovery),
        "effective_risk_transitions": fwd(er), "effective_recovery_transitions": fwd(ec),
        "repeated_risk_triggers": len(risk) - len(er), "repeated_recovery_triggers": len(recovery) - len(ec),
        "half_state_hours": int(half.sum()), "half_state_market_return_arithmetic": carry,
        "full_exposure_equivalent_hours_removed": .5 * int(half.sum()),
        "market_carry_removed": .5 * carry, "incremental_fees": fee_delta, "arithmetic_net_delta": observed,
    }
...
def run(d):
    ...
    met = {k: {s: metrics(arrays[k], p[k], span) for s, span in (("training", TRAIN), ("development_oos", OOS), ("full_scored", FULL))} for k in p}
    cb, bb = breadth(arrays["candidate"][3], d.index), breadth(arrays["b1"][3], d.index)
    ...
    gates = {
        ...
        "positive_fold_concentration_not_above_50pct": cb["positive_fold_concentration"] is not None and cb["positive_fold_concentration"] <= .5,
    ...
    return {"metrics": met, "breadth": {"candidate": cb, "b1": bb, "residual_sharpe_vs_b1": sharpe(residual)}, "bootstrap": boot, "diagnostics": diagnose(p, arrays, events), "acceptance_gates": gates, "accepted": all(gates.values())}


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--btc", type=Path, required=True); ap.add_argument("--eth", type=Path, required=True); ap.add_argument("--output", type=Path, required=True)
    a = ap.parse_args()
    out = {"family_id": "sign-transition-entropy-persistence-overlay-1h-v1", "issue": 705, "candidate_count": 1, "parameter_grid_count": 0, "fee_one_way": FEE, "research_parent": "5a0fcc97d1a882f8223656c51f5bb8055f534e38", "source": {"provider": "OKX SPOT public confirmed 1H", "hashes": HASHES, "rows_in_source": 43941, "scored_prefix_rows": N}, "sample": {"training": TRAIN, "development_oos": OOS, "full_scored": FULL}, "markets": {}}
    ...
    out["verdict"] = "nominate_exact_sign_transition_entropy_persistence_overlay_for_replication" if out["accepted"] else "reject_exact_sign_transition_entropy_persistence_overlay_family"
