...
def main() -> int:
    ...
    momentum_lookbacks = [
        int(value) for value in search.get("momentum_lookbacks", [30, 90, 180])
    ]
    reversal_lookbacks = [
        int(value) for value in search.get("reversal_lookbacks", [2, 5, 10])
    ]
    trend_weights = [
        float(value) for value in search.get("trend_weights", [0.55, 0.70, 0.85])
    ]
    ...
    manifest_path, manifest_appended = append_experiment_manifest(
