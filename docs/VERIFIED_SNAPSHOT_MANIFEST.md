...
helper 只证明本地 CSV 字节与本地 metadata 声明一致。它不会联网证明 provider、workflow、artifact 或 commit 标识真实且互相对应，也不会根据 `timeframe` 自动推导 cadence。可审计复现仍须保留原始 OKX 分页响应、snapshot metadata、下载请求、workflow run、artifact ID、exact head 和下载包哈希。
