# Executable OKX instrument, quote, and order-constraint evidence gate

This guide documents the public, read-only instrument, depth-one quote, and pre-adapter order-constraint
boundaries implemented on current `main`. It verifies pinned provider bytes, archives one deterministic
instrument observation, reconstructs one complete OKX top-of-book timing envelope, and invokes the
implemented submission-time quantity and limit-price constraint gate. It does not connect to an
account, read balances or positions, or submit an order.
...
8. serializes and replays `ReconstructableOKXTopOfBookEvidence` with identical canonical bytes and
   evidence ID;
9. invokes `validate_okx_spot_limit_order_constraints()` at a deterministic submission timestamp;
10. proves the proposed base quantity is canonical, at least `minSz`, and an exact `lotSz` multiple;
11. proves the proposed limit price is canonical and an exact `tickSz` multiple;
12. derives observed spread and quote-equivalent arithmetic for the minimum base quantity while
...
constraint_probe.base_quantity=0.00001
...
## Constraint-probe scope

The example uses the replayed instrument snapshot, its minimum base quantity, and the observed ask as
one deterministic limit-price probe. Passing means only that:

- the instrument snapshot was available and within the configured local-age limit at the supplied
  submission timestamp;
- its conservative exchange-time upper bound had not reached `valid_until_utc`;
- the base quantity passed canonical encoding, minimum-size, and lot-size checks;
- the limit price passed canonical encoding and tick-size alignment.

The observed ask is used only as a real, immutable tick-alignment input. It is not treated as a fill,
broker acceptance, minimum quote-notional rule, or price recommendation.
...
therefore remain ineligible for a paper order even though each individual record reconstructs and the
constraint probe passes.

An exact instrument archive retry is idempotent. A conflicting content-addressed file is an
operational blocker and must not be overwritten manually. The quote replay and constraint probe are
canonical in memory; this command does not claim to provide a durable paper decision, order, fill,
cash, portfolio, reconciliation, or PnL journal.

## What remains blocked

This evidence gate is not a complete order-sizing approval. Current `main` validates base quantity,
instrument freshness/validity, lot size, minimum base size, and limit-price tick alignment. It does
not expose a public minimum quote-notional constraint, infer deeper-book capacity, approve cash or
portfolio risk, persist a broker event, or reconcile resulting state.
...
- the instrument is not `live`, has not begun continuous trading, is expired, or has reached
  `valid_until_utc`;
- the instrument snapshot is stale at submission;
- the quote is stale, crossed, malformed, from another instrument, or not received before the paper
  decision;
- base quantity is noncanonical, below `minSz`, or not an exact `lotSz` multiple;
- limit price is noncanonical or not an exact `tickSz` multiple;
- a required minimum quote-notional rule is unavailable;
- risk-approved cash, portfolio state, broker event persistence, or reconciliation evidence is
  absent.

The executable example is an evidence, replay, and pre-adapter constraint gate only. Its stdout is not
an order intent, exchange acknowledgement, fill, cash ledger, portfolio transition, or PnL record.
