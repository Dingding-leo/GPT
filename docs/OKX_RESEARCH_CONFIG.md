...
| `transaction_cost_bps` | number | 非布尔、有限且非负，按每单位绝对仓位变化的单边基点数解释。当前 rolling 基线将该字段定义为交易所手续费假设。 |
...
基线 `1.0` 对应单边 `5 bps`，并在该费用下执行完整 walk-forward 候选选择。`1.5 / 2.0 / 3.0` 对应单边 `7.5 / 10 / 15 bps`，只对已选路径重新计价；它们不是在每个成本水平重新搜索参数，也不是对 spread、slippage、market impact 或 latency 的独立测量。
...
当前 `config/okx_holdout.json` 复用相同的 strategy 与候选族，并声明：
