...
3. 在任何第三方安装之前，验证根目录、静态依赖语法及受信任的 direct-dependency scope allowlist；
...
5. 记录 approved/declared direct names、解析出的精确版本、PyPI artifact URL 和 SHA-256；
...
## 受信任的 direct-dependency scope policy

通过静态语法检查还不够。受信任的 `scripts/dependency_audit_direct_policy.py` 会把 distribution name 转为小写，并把连续的 `-`、`_`、`.` 统一为 `-`，然后按声明 scope 核对名称：

- build scope（`[build-system].requires`）只批准 `setuptools`；
- project scope（`[project].dependencies` 与所有 optional-dependency 列表的并集）只批准 `numpy`、`pandas`、`pytest`、`ruff`。

allowlist 约束的是规范化后的**直接包名和 scope**，不是版本范围。已批准名称的版本变化仍会进入后续跨平台解析、artifact 记录和 vulnerability audit；但新增未知名称，或把已批准名称移到错误 scope，例如把 `numpy` 放入 build scope、把 `setuptools` 放入 project/optional scope，都会在生成持久化 resolver 输入和任何第三方安装之前失败。

成功时会写出 `reports/security/direct-dependency-policy.json`，包含：

- `schema_version: 1`；
- `approved_direct_dependencies`：受信任策略按 scope 批准的规范名称；
- `declared_direct_dependencies`：当前 proposed `pyproject.toml` 按 scope 实际声明的规范名称。

该文件是本次门禁的策略证据，不是给 PR 作者自行扩展的配置。新增 direct dependency 必须先在受信任 base policy 中获得明确审查和批准；不能只修改 PR head 的 `pyproject.toml` 来绕过。
...
- 未获批准的 direct package name，或已批准名称出现在错误的 build/project scope。
...
这些命令验证受信任解析、direct-name scope policy 和工作流声明，不会在本地复现 GitHub Actions 的全部 20 个 Python/平台解析任务，也不会替代远端 vulnerability audit。

## PR 作者提交前检查

在修改 `pyproject.toml` 的 pull request 中：

1. 先运行上面的 direct-policy 与 `prepare` 命令，并审阅两份 JSON 中的 approved、declared 与规范化 requirements；
2. 确认没有新增未批准名称、cross-scope 移动、legacy build file、dynamic metadata、`cmdclass`、本地路径或 direct URL；
...
通过门禁表示：受信任解析器能够静态读取声明；所有 direct dependency name 位于当前批准的 scope；解析结果来自 public PyPI binary artifacts；记录的 artifact SHA-256 格式有效；两组锁定依赖在当次 `pip-audit` 数据下未触发门禁失败。
