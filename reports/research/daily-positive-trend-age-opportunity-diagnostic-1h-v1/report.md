...
**Remaining blocker:** continuous price-path states tested so far either lack active-state support or produce weak, regime-dependent ordering. Improving selector quality now requires a materially new information channel rather than another transform of endpoint direction, margin, path shape or elapsed time.

**Next strategy experiment:** preregister one training-only **volume-participation trend-quality diagnostic**. At every completed daily B1-long decision, measure one fixed own-history state: the latest 168H quote-volume-weighted signed return divided by total quote volume, standardised by the latest 168H absolute-return scale. Test next-24H active-B1 gross opportunity and adverse excursion with the same non-overlapping daily labels, seven-day dependence blocks and bilateral support gates. Use no threshold, no OOS and no executable candidate unless the volume-information premise passes.
