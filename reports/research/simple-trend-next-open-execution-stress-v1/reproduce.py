...
def main() -> None:
    ...
    series = {}
    result = {'markets': {}, 'bootstrap': {}, 'fee_one_way_bps': 5.0, 'lookback_hours': 2160}
    for market, root in roots.items():
        verify(root)
        snap = next((root / 'snapshot').glob('*.csv'))
        df = pd.read_csv(snap, parse_dates=['timestamp'])
        assert len(df) == 43930 and df.timestamp.is_unique and df.timestamp.is_monotonic_increasing
        ...
        assert np.max(
            np.abs(paths['C0'][0] - wf.benchmark_simple_trend_long_cash_return.to_numpy())
        ) < 1e-12
        fresh = {
    ...
    for market, paths in series.items():
        for x, y in pairs:
            observed[f'{market}_{x}-{y}_mean'] = float(np.mean(paths[x][0] - paths[y][0]) * H)
