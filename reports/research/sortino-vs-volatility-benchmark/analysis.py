...
CANONICAL_SIGNATURE = (
    "sortino-vs-volatility-benchmark-v1|markets=BTC-USDT,ETH-USDT|"
    "source=persisted-net-rolling-oos-returns|benchmark=volatility-targeted-long|"
    "metric=annualized-sortino-zero-mar|"
    "downside-deviation=sqrt(mean(min(return,0)^2))|"
    "claim=strategy-minus-benchmark-sortino>0-in-both-markets|"
    "resampling=paired-noncircular-moving-block-bootstrap|block-length=20-sessions|"
    "resamples=2000|confidence=0.95|annualization=365|"
...
def annualized_sortino(returns: np.ndarray, annualization: int = ANNUALIZATION) -> float:
    ...
    if downside_deviation == 0.0:
        ...
        if mean_return < 0.0:
            return -math.inf
        return 0.0
    return mean_return * math.sqrt(annualization) / downside_deviation
...
def bootstrap_sortino_delta(
    ...
    *,
    ...
    observed_strategy = annualized_sortino(strategy, annualization)
    observed_benchmark = annualized_sortino(benchmark, annualization)
    ...
    for index in range(resamples):
        ...
        deltas[index] = annualized_sortino(
            strategy[sample_indices], annualization
        ) - annualized_sortino(benchmark[sample_indices], annualization)

    alpha = 1.0 - float(confidence)
    lower, upper = np.quantile(deltas, [alpha / 2.0, 1.0 - alpha / 2.0])
    return {
        "strategy_sortino": observed_strategy,
        "benchmark_sortino": observed_benchmark,
...
def analyze_market(path: str | Path, market: str) -> dict[str, object]:
    ...
    result = bootstrap_sortino_delta(
...
def build_result(artifact_dir: str | Path) -> dict[str, object]:
    ...
    if failed_markets:
        rejection_reason = (
            ...
            "benchmark Sortino delta was non-positive in: " + ", ".join(failed_markets)
        )

    return {
        "canonical_signature": CANONICAL_SIGNATURE,
        "hypothesis": (
            "The strategy has a higher annualized zero-MAR Sortino ratio than the "
        ...
        "settings": {
            ...
            "downside_deviation": "sqrt(mean(min(return, 0)^2))",
            "seeds": SEEDS,
        },
        "provenance": {
            "provider": "OKX",
            "market_type": "spot",
            "timeframe": "1Dutc",
            "source_workflow_run": 29940617808,
