...
SIGNATURE = (
    "lagged-asset-autocorrelation-regime-consistency-v1|"
    "markets=BTC-USDT,ETH-USDT|"
    "source=persisted-net-rolling-oos-asset-and-strategy-returns|"
    "regime-statistic=lag1-pearson-autocorrelation-of-prior-60-asset-returns|"
...
def load_market_inputs(
    ...
    *,
    ...
    missing = {"timestamp", "asset_return", "strategy_return", "fold"} - set(returns)
    if missing:
    ...
    index = explicit_daily_utc_index(returns["timestamp"], label="return")
    asset_returns = pd.to_numeric(returns["asset_return"], errors="raise").to_numpy(dtype=float)
    ...
    if sorted(frame["fold"].unique().tolist()) != fold_ids:
...
def lag1_autocorrelation(values: np.ndarray) -> float:
    if len(values) < 3:
        raise ValueError("lag-1 autocorrelation requires at least three observations")
    earlier = np.asarray(values[:-1], dtype=float)
    later = np.asarray(values[1:], dtype=float)
    if not np.isfinite(earlier).all() or not np.isfinite(later).all():
        raise ValueError("autocorrelation inputs must be finite")
    if float(np.std(earlier, ddof=0)) == 0.0 or float(np.std(later, ddof=0)) == 0.0:
        return float("nan")
    return float(np.corrcoef(earlier, later)[0, 1])


def classify_autocorrelation_regimes(
    returns_frame: pd.DataFrame,
    *,
    lookback: int = AUTOCORRELATION_LOOKBACK,
    require_both_regimes: bool = True,
) -> pd.DataFrame:
    if isinstance(lookback, bool) or not isinstance(lookback, int) or lookback < 3:
        raise ValueError("autocorrelation lookback must be an integer of at least three")
    if not isinstance(returns_frame.index, pd.DatetimeIndex):
        raise TypeError("return frame must use a DatetimeIndex")
    if "asset_return" not in returns_frame:
        raise ValueError("return frame is missing asset_return")

    lagged = returns_frame["asset_return"].shift(1)
    autocorrelation = lagged.rolling(lookback, min_periods=lookback).apply(
        lag1_autocorrelation,
        raw=True,
    )
    classified = returns_frame.copy()
    classified["lagged_autocorrelation"] = autocorrelation
    classified = classified.loc[np.isfinite(classified["lagged_autocorrelation"])].copy()
    classified["regime"] = np.where(
        classified["lagged_autocorrelation"].to_numpy(dtype=float) > 0.0,
        "positive",
        "nonpositive",
    )
    if len(classified) != len(returns_frame) - lookback:
        raise ValueError("unexpected unavailable lagged-autocorrelation observations")
    if require_both_regimes and set(classified["regime"]) != {"positive", "nonpositive"}:
        raise ValueError("classified rows must contain both autocorrelation regimes")
    return classified
...
def conditional_annualized_means(frame: pd.DataFrame) -> dict[str, float]:
    ...
    for regime in ("positive", "nonpositive"):
        ...
        if not len(values):
            raise ValueError(f"sample contains no {regime}-autocorrelation observations")
        means[regime] = float(values.mean() * ANNUALIZATION)
    return means


def analyze_market(returns_frame: pd.DataFrame, *, seed: int) -> dict[str, object]:
    classified = classify_autocorrelation_regimes(returns_frame)
    ...
    distributions = {
        "positive": np.empty(RESAMPLES),
        "nonpositive": np.empty(RESAMPLES),
    }
    ...
    autocorrelation = classified["lagged_autocorrelation"].to_numpy(dtype=float)
    return {
        "observations": len(classified),
        "excluded_warmup_observations": AUTOCORRELATION_LOOKBACK,
        "seed": seed,
        "autocorrelation_summary": {
            "minimum": float(autocorrelation.min()),
            "median": float(np.median(autocorrelation)),
            "maximum": float(autocorrelation.max()),
        },
        "regimes": regimes,
...
def build_result(artifact_dir: Path) -> dict[str, object]:
    ...
    for market, specification in MARKETS.items():
        returns_frame = load_market_inputs(
            artifact_dir,
            market=market,
        ...
        result = analyze_market(returns_frame, seed=specification["seed"])
        market_results[market] = result
        for regime, regime_result in result["regimes"].items():
            if not regime_result["passes"]:
                failures.append(
                    f"{market} {regime}-autocorrelation mean 95% lower bound is non-positive: "
    ...
    return {
        ...
        "candidates": [
            {
                "name": "lagged-asset-autocorrelation-regime-consistency",
        ...
        "hypothesis": (
            ...
            "arithmetic means in both positive and non-positive prior asset-return "
            "autocorrelation regimes."
        ),
        "economic_rationale": (
            "The strategy combines trend and reversal signals. A credible composite should not "
            "require only persistent or only mean-reverting recent market dynamics to produce "
            "positive net returns."
        ),
        "method": {
            "annualization": ANNUALIZATION,
            "autocorrelation_statistic": (
                "Pearson lag-1 autocorrelation within the prior 60 asset returns"
            ),
            "current_session_excluded": True,
            "positive_regime_rule": "lagged autocorrelation is greater than zero",
            "nonpositive_regime_rule": "lagged autocorrelation is less than or equal to zero",
        ...
        "limitations": [
            ...
            "Observed regime labels are treated as fixed during resampling, so regime-estimation "
            "uncertainty is not separately modeled.",
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Test net-return consistency across lagged autocorrelation regimes."
    )
