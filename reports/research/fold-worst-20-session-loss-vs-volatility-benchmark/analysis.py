...
def fold_worst_window_deltas(
    ...
    *,
    ...
    for fold in complete_fold_ids(frame, fold_length=fold_length):
        ...
        strategy_worst = worst_compounded_window_return(
            fold_frame[STRATEGY_RETURN_COLUMN].to_numpy(dtype=float),
            window=window,
        )
        benchmark_worst = worst_compounded_window_return(
            fold_frame[BENCHMARK_RETURN_COLUMN].to_numpy(dtype=float),
            window=window,
        )
        rows.append(
            {
                ...
                "benchmark_worst_window_return": benchmark_worst,
...
def analyze_market(artifact_dir: str | Path, market: str) -> dict[str, object]:
    ...
    return {
        **bootstrap,
        ...
        "benchmark_mean_fold_worst_window_return": float(
            fold_metrics["benchmark_worst_window_return"].mean()
        ),
...
def build_result(artifact_dir: str | Path) -> dict[str, object]:
    ...
    return {
        ...
        "hypothesis": (
            ...
            "20-session return than volatility-targeted long in both BTC-USDT and ETH-USDT."
        ),
        ...
        "settings": {
            ...
            "aggregate_metric": "mean strategy-minus-benchmark fold delta",
