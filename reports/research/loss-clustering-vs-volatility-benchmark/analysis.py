...
def sampled_block_indices(
    ...
    if lengths[-1] == 1:
        if len(lengths) < 2 or lengths[-2] <= 2:
            raise ValueError(
                "observation_count cannot be partitioned into sampled blocks of at least two"
            )
        lengths[-2] -= 1
        lengths[-1] += 1
...
def build_result(artifact_dir: str | Path) -> dict[str, object]:
    ...
    return {
        ...
        "provenance": {
            ...
            "current_main_commit": "546aa5034c61c6dd13262199eb52910add93f5a6",
        },
    }
