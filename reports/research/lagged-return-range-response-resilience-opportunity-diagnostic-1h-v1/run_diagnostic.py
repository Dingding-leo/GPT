...
def parse_checksum(payload: bytes, url: str) -> tuple[str, str]:
...
def download(spec: ObjectSpec) -> Downloaded:
    expected, checksum_text = parse_checksum(http_get(spec.checksum_url), spec.checksum_url)
...
def parse_kline(downloaded: Downloaded) -> pd.DataFrame:
    ...
    unit = "us" if frame["open_time"].max() >= 10**15 else "ms"
    frame.insert(0, "timestamp", pd.to_datetime(frame["open_time"], unit=unit, utc=True))
...
def build_labels(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    for target_day in score_days:
        ...
        feature_start = entry - FEATURE_HOURS
        if feature_start < 1:
            raise ValueError(f"insufficient feature history at {target_day}")
        predictor = returns[feature_start : entry - 1]
        response = log_range[feature_start + 1 : entry]
        if len(predictor) != PAIR_COUNT or len(response) != PAIR_COUNT:
            raise ValueError(f"wrong pair count at {target_day}")
        state = correlation(predictor, response)
        ...
        gross = float(path[-1])
        window_returns = returns[feature_start:entry]
        feature_return = float(close[entry - 1] / close[feature_start] - 1.0)
        feature_rv = float(np.sqrt(np.mean(np.square(window_returns))))
        negative = predictor < 0
        positive = predictor > 0
        negative_response = (
            float(np.mean(response[negative])) if int(np.sum(negative)) >= 3 else math.nan
        )
        positive_response = (
            float(np.mean(response[positive])) if int(np.sum(positive)) >= 3 else math.nan
        )
        rows.append(
            {
                "decision_day": target_day,
                "state": state if state is not None else math.nan,
                "feature_return_168h": feature_return,
                "feature_rv_168h": feature_rv,
                "negative_next_range_mean": negative_response,
                "positive_next_range_mean": positive_response,
                "gross": gross,
                "net": gross - 2 * FEE_ONE_WAY,
                "adverse": float(path.min()),
                "valid": bool(state is not None and math.isfinite(state)),
            }
        )
...
def standardized_slope(state: np.ndarray, target: np.ndarray) -> float | None:
    ...
    value = float(np.dot(standardized, centered_target) / denominator)
    return value if math.isfinite(value) else None
...
def segment_slope(part: pd.DataFrame, target: str) -> float | None:
    ...
    return standardized_slope(
        valid["state"].to_numpy(float), valid[target].to_numpy(float)
    )
...
def quintile_statistics(valid: pd.DataFrame) -> dict[str, Any]:
    ordered = valid.sort_values(["state", "decision_day"], kind="mergesort").reset_index(
        drop=True
    )
...
def point_statistics(table: pd.DataFrame) -> dict[str, Any]:
    ...
    feature_return = valid["feature_return_168h"].to_numpy(float)
    feature_rv = valid["feature_rv_168h"].to_numpy(float)
    negative_response = valid["negative_next_range_mean"].to_numpy(float)
    positive_response = valid["positive_next_range_mean"].to_numpy(float)
    return {
        ...
        "descriptive_attribution": {
            "state_feature_return_rho": spearman(state, feature_return),
            "state_feature_rv_rho": spearman(state, feature_rv),
            "mean_negative_next_range": float(np.nanmean(negative_response)),
            "mean_positive_next_range": float(np.nanmean(positive_response)),
            "mean_response_gap_positive_minus_negative": float(
                np.nanmean(positive_response - negative_response)
            ),
        },
        "target_economics": {
...
def bootstrap_indices(length: int) -> np.ndarray:
    ...
    starts = np.arange(length - BLOCK_DAYS + 1)
    blocks_needed = math.ceil(length / BLOCK_DAYS)
    chosen = rng.choice(starts, size=(RESAMPLES, blocks_needed), replace=True)
    offsets = np.arange(BLOCK_DAYS)
    return (chosen[:, :, None] + offsets[None, None, :]).reshape(RESAMPLES, -1)[
        :, :length
    ]
...
def bootstrap(table: pd.DataFrame, indices: np.ndarray) -> dict[str, Any]:
    ...
    for number, sampled in enumerate(indices):
        ...
        if len(sampled_state) < 250 or float(np.std(sampled_state)) == 0:
            continue
        sampled_gross = gross[sampled][mask]
        sampled_adverse = adverse[sampled][mask]
        if float(np.std(sampled_gross)) == 0 or float(np.std(sampled_adverse)) == 0:
            continue
        values = (
...
def common_inference(bootstraps: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    common = np.logical_and.reduce(
        [bootstraps[name]["valid_mask"] for name in market_names]
    )
    if not bool(common.any()):
        raise ValueError("no common valid bootstrap draws")
    stacked = np.stack(
        [bootstraps[name]["draws"][common] for name in market_names], axis=0
    )
...
def strip_bootstrap_arrays(result: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        if key not in {"draws", "valid_mask"}
    }
...
def evaluate_gates(result: dict[str, Any], common: dict[str, Any]) -> dict[str, bool]:
    ...
    return {
        ...
        "state_support": result["state_quantiles"]["iqr"] >= 0.10,
...
def render_report(evidence: dict[str, Any]) -> str:
    lines = [
        "# Lagged signed-return/range-response resilience diagnostic",
        "",
        "```text",
        f"family          {FAMILY_ID}",
        "candidate count 0",
    ...
    if evidence["source"]["failures"]:
        ...
        for failure in evidence["source"]["failures"]:
            lines.append(
                f"- `{failure['market']} {failure['period']}`: {failure['error']}"
            )
        lines.extend(
            [
                "",
    ...
    lines.extend(
        [
            ...
            "## Standardised slopes",
            "",
            "| Market | Gross slope (95% CI) | Adverse slope (95% CI) |",
            "|---|---:|---:|",
        ]
    )
    for market, result in evidence["markets"].items():
        lines.append(
            f"| {market} | {result['gross_slope_per_state_sd']:+.4%} "
            f"[{result['uncertainty']['gross_slope_ci95'][0]:+.4%},"
            f"{result['uncertainty']['gross_slope_ci95'][1]:+.4%}] | "
            f"{result['adverse_slope_per_state_sd']:+.4%} "
            f"[{result['uncertainty']['adverse_slope_ci95'][0]:+.4%},"
            f"{result['uncertainty']['adverse_slope_ci95'][1]:+.4%}] |"
        )
    lines.extend(
        [
            "",
    ...
    lines.extend(
        [
            ...
            "",
            "| Market | Mean gross | Mean net | Mean adverse | Positive gross | "
            "Turnover | Fees |",
            "|---|---:|---:|---:|---:|---:|---:|",
        ]
    )
...
def main() -> None:
    ...
    if failed:
        ...
        verdict = (
            ...
            if accepted
            else "reject_lagged_return_range_response_resilience_information_premise"
        )
        evidence = {
            "family_id": FAMILY_ID,
            "classification": "training-only own-OHLCV information eligibility diagnostic",
    ...
    manifest_digest = hashlib.sha256(
        (output_dir / "source-manifest.json").read_bytes()
    ).hexdigest()
    evidence["source"]["manifest_sha256"] = manifest_digest
