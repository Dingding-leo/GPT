...
def target_position(prices: pd.Series) -> pd.Series:
    ...
    risk_scale = log_returns.rolling(VOLATILITY_LOOKBACK, min_periods=VOLATILITY_LOOKBACK).std(
        ddof=0
    )
...
def paired_bootstrap_comparison(
    ...
    *,
    ...
    for sample_index in range(resamples):
        ...
        drawdown_deltas[sample_index] = max_drawdown(sampled_adaptive) - max_drawdown(sampled_fixed)
...
def build_result(artifact_dir: Path) -> dict[str, Any]:
    ...
    for market, evidence in MARKETS.items():
        ...
        adaptive_returns = pd.to_numeric(persisted["strategy_return"], errors="raise").to_numpy(
            dtype=float
        )
